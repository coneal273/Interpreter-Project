...
1. comparison operators (B 80%)
2. full math expressions (B+ 87%)
3. if expressions (A 93%)
4. The whole deal (A+ 100%)
...
Do cuts 1,2, and 3. In addition, write an executable program called `asac`. This program will be called as follows

```
> asac file1.asa
```

The first arguement is a filename to Asa source code. The program will read the file into memory, lex, parse, and interpret it. Start any main() function that exists. If none exists, the program falls starts a REPL that allows you to interact with the compiled code.
